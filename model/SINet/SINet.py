...
from model.SINet.ResNet import ResNet_2Branch
...
class SINet_ResNet50(nn.Module):
    ...
    def forward(self, x):
        ...
        # - low-level features
        x0 = self.backbone.maxpool(x0)    # (BS, 64, 88, 88)
        x1 = self.backbone.layer1(x0)     # (BS, 256, 88, 88)
        ...
        x2_sm = x2                              # (512, 44, 44)
        x3_sm = self.backbone.layer3_1(x2_sm)     # (1024, 22, 22)
        x4_sm = self.backbone.layer4_1(x3_sm)     # (2048, 11, 11)
        ...
        x3_im = self.backbone.layer3_2(x2_sa)                 # (1024, 22, 22)
        x4_im = self.backbone.layer4_2(x3_im)                 # (2048, 11, 11)
    ...
    def initialize_weights(self):
        ...
        for k, v in self.backbone.state_dict().items():
            if k in pretrained_dict.keys():
                ...
                v = pretrained_dict[name]
                all_params[k] = v
        assert len(all_params.keys()) == len(self.backbone.state_dict().keys())
